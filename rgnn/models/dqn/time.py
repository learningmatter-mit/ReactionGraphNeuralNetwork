...
class TNet(torch.nn.Module, Configurable, ABC):
    ...
    def __init__(
        ...
        tau: float = 15,
        T_scaler_m: float = 7918.951990135471, 
    ...
    def forward(self, data: DataDict, temperature: float | None = None, defect: float | None = None):
        ...
        self.scaler = torch.exp(self.T_scaler_m*(1/batch_temperature-1/500))  #referenced to 500 K
    ...
    @classmethod
    def load_representation(cls, reaction_model, n_feat, dropout_rate, tau, T_scaler_m, D_scaler, pooling):
        ...
        model = cls(**input_params, n_feat=n_feat, dropout_rate=dropout_rate, tau=tau, T_scaler_m=T_scaler_m, D_scaler=D_scaler, pooling=pooling)
...
class TNetEmb(torch.nn.Module, Configurable, ABC):
    ...
    def forward(self, data: DataDict, temperature: float | None = None, defect: float | None = None):
        ...
        self.scaler = torch.exp(self.T_scaler_m*(1/batch_temperature-1/500))  #referenced at 500 K
        time = F.tanh(F.softplus(self.time_out(graph_out)).squeeze(-1))
