...
@registry.register_model("base_q")
class BaseDQN(torch.nn.Module, Configurable, ABC):
    ...
    def get_change_n_elems(self, data):
        ...
        for _, graph_id in enumerate(torch.unique(data["batch"])):
            ...
            for specie in self.reaction_model.atomic_numbers:
                if specie not in elem_change_dict.keys():
                    elem_change_dict[specie] = 0
...
@registry.register_model("base_ac")
class BaseActorCritic(torch.nn.Module, Configurable, ABC):
    """Base class for energy models.
    Takes data or batch and returns energy.
    ...
    @classmethod
    def load(cls, path: str) -> "BaseActorCritic":
        """Load the model from checkpoint created by pytorch lightning.
